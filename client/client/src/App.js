...
import { LogOut, Book, Send, Paperclip, BarChart2, Calendar, Calculator, GraduationCap, CheckCircle, XCircle, ArrowUp, FileText, Bell, Coffee, Clock, PlusCircle } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-96 border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {isRegister && <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm" placeholder="Full Name" onChange={e => setFormData({...formData, name: e.target.value})} />}
          <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm" placeholder="Email Address" onChange={e => setFormData({...formData, email: e.target.value})} />
          <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm" type="password" placeholder="Password" onChange={e => setFormData({...formData, password: e.target.value})} />
          <button className="w-full bg-black text-white py-3 rounded-lg hover:bg-gray-800 font-bold text-sm transition-all">{isRegister ? 'Join Campus' : 'Sign In'}</button>
        </form>
        <button onClick={() => setIsRegister(!isRegister)} className="w-full text-center mt-6 text-xs text-gray-500 hover:text-black font-medium">{isRegister ? "Have an account? Login" : "New student? Register"}</button>
      </div>
    </div>
  );
};
...
const CampusMode = ({ user }) => {
    const [section, setSection] = useState('buzz'); 
    ...
    useEffect(() => { API.get(`/posts?section=${section === 'notices' ? 'notice' : 'campus'}`).then(res => setPosts(res.data)); }, [section]);
    ...
    const submitPost = async () => {
        ...
        setPosts([res.data, ...posts]); setPostContent(''); setPostFile(null); setIsPollMode(false);
    };
    const handleVote = async (pid) => { const res = await API.put(`/posts/${pid}/vote`); setPosts(posts.map(p=>p._id===pid?{...p, votes:res.data.votes}:p)); };
    const handlePollVote = async (pid, idx) => { const res = await API.put(`/posts/${pid}/poll`, {optionIndex: idx}); setPosts(posts.map(p=>p._id===pid?{...p, pollOptions:res.data.pollOptions}:p)); };

    return (
        <div className="flex h-full">
            <div className="w-64 bg-white border-r border-gray-100 flex flex-col pt-6">
                <div className="px-6 mb-6"><h2 className="text-xl font-black text-gray-900">Campus Life</h2></div>
                <div className="space-y-1 px-4">
                    <button onClick={()=>setSection('notices')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold ${section==='notices'?'bg-red-50 text-red-600':'text-gray-500'}`}><Bell size={16} className="inline mr-2"/> Notice Board</button>
                    <button onClick={()=>setSection('buzz')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold ${section==='buzz'?'bg-blue-50 text-blue-600':'text-gray-500'}`}><Coffee size={16} className="inline mr-2"/> The Buzz</button>
                </div>
            </div>
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
                        ...
                        <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-50">
                            <div className="flex gap-3"><button onClick={()=>setIsPollMode(!isPollMode)} className="text-gray-400 hover:text-blue-600"><BarChart2 size={20}/></button><label className="cursor-pointer text-gray-400 hover:text-blue-600 flex items-center gap-2 text-xs font-bold"><Paperclip size={20}/> {postFile && <span className="text-blue-600 truncate max-w-[100px]">{postFile.name}</span>}<input type="file" className="hidden" onChange={e => setPostFile(e.target.files[0])} /></label></div>
...
const AcademicMode = ({ user, subjects, activeSubject, setActiveSubject }) => {
    ...
    useEffect(() => {
        ...
        API.get(`/posts?subjectId=${activeSubject._id}`).then(res => setPosts(res.data));
        ...
        API.get('/attendance').then(res => { const record = res.data.find(r => r.subjectId === activeSubject._id); setAttendance(record || { attended: 0, total: 0 }); });
    ...
    const submitDeadline = async () => {
        ...
        setDeadlines([...deadlines, res.data]); setNewDeadlineTitle(''); setNewDeadlineDate('');
    };
    const handleVote = async (pid) => { const res = await API.put(`/posts/${pid}/vote`); setPosts(posts.map(p=>p._id===pid?{...p, votes:res.data.votes}:p)); };
    const handlePollVote = async (pid, idx) => { const res = await API.put(`/posts/${pid}/poll`, {optionIndex: idx}); setPosts(posts.map(p=>p._id===pid?{...p, pollOptions:res.data.pollOptions}:p)); };
    const markAttendance = async (type) => { const newAttended = type === 'present' ? attendance.attended + 1 : attendance.attended; const newTotal = attendance.total + 1; setAttendance({ attended: newAttended, total: newTotal }); await API.put('/attendance', { subjectId: activeSubject._id, type }); };
    ...
    const saveTimetable = async () => { await API.post('/timetable', { timetable }); alert("Saved"); };
                    ...
                    {tab === 'feed' ? (
                        <>
                            <div className="flex-1 overflow-y-auto p-8 bg-gray-50/50">
                                {/* DEADLINES WIDGET - ALWAYS VISIBLE */}
                                <div className="mb-8 p-4 bg-yellow-50 border border-yellow-100 rounded-xl">
                                    <h4 className="text-xs font-bold text-yellow-700 uppercase mb-2 flex items-center gap-2"><Clock size={14}/> Upcoming Deadlines</h4>
                                    
                                    <div className="space-y-2 mb-3">
                                        {deadlines.length > 0 ? deadlines.map(d=>(<div key={d._id} className="flex justify-between text-sm bg-white p-2 rounded shadow-sm text-gray-800"><span>{d.title}</span><span className="font-bold text-red-500">{differenceInDays(new Date(d.dueDate), new Date())} days left</span></div>)) : <p className="text-xs text-yellow-600 italic">No pending deadlines.</p>}
                                    </div>

                                    <div className="flex gap-2 border-t border-yellow-200 pt-2">
                                        <input className="w-full p-2 rounded text-xs border border-yellow-300 bg-white" placeholder="Task Name..." value={newDeadlineTitle} onChange={e=>setNewDeadlineTitle(e.target.value)}/>
                                        <input type="date" className="p-2 rounded text-xs border border-yellow-300 bg-white" value={newDeadlineDate} onChange={e=>setNewDeadlineDate(e.target.value)}/>
                                        <button onClick={submitDeadline} className="bg-yellow-600 text-white p-2 rounded hover:bg-yellow-700 transition"><PlusCircle size={14}/></button>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-50">
                                        <div className="flex gap-2">
                                            ...
                                            <label className="cursor-pointer text-gray-400 hover:text-blue-600 flex items-center gap-2">
                                                <Paperclip size={20}/> {postFile && <span className="text-xs text-blue-600 font-bold max-w-[80px] truncate">{postFile.name}</span>}
